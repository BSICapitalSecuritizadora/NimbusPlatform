@php
    $heading = $this->getHeading() ?? 'Visão Operacional';
    $description = $this->getDescription() ?? 'Situação atual das obrigações que exigem acompanhamento operacional.';
    $summary = $this->getSummaryData();
    $cards = $this->getOverviewCards();
    $alerts = $this->getAlertItems();
    $secondary = $this->getSecondaryCards();
    $hasAlerts = collect($alerts)->contains(fn ($a) => (int) $a['raw_value'] > 0);
@endphp

<x-filament-widgets::widget class="bsi-cockpit-widget bsi-obligation-overview-widget">
    <x-filament::section
        :heading="$heading"
    ...
    >
        <x-slot name="afterHeader">
            ...
            <div class="hidden sm:flex flex-wrap items-center gap-2 text-xs">
                <span class="inline-flex items-center gap-1.5 rounded-full border border-gray-200/80 bg-bsi-paper px-2.5 py-0.5 font-medium text-gray-700 dark:border-white/10 dark:bg-[#06151c]/80 dark:text-gray-300">
                    <span class="size-1.5 rounded-full bg-bsi-gold-500"></span>
                    <span><strong class="font-semibold text-gray-900 dark:text-white">{{ $summary['total_formatted'] }}</strong> na carteira</span>
                </span>

                @if((int) $summary['vencida'] > 0)
                    <span class="inline-flex items-center gap-1.5 rounded-full border border-danger-300/80 bg-danger-50 px-2.5 py-0.5 font-medium text-danger-800 dark:border-danger-500/30 dark:bg-danger-500/15 dark:text-danger-200">
                        <span class="size-1.5 rounded-full bg-danger-500 animate-pulse"></span>
                        <span><strong class="font-semibold">{{ $summary['vencida_formatted'] }}</strong> em atraso</span>
                    </span>
                @else
                    <span class="inline-flex items-center gap-1.5 rounded-full border border-emerald-300/80 bg-emerald-50 px-2.5 py-0.5 font-medium text-emerald-800 dark:border-emerald-500/30 dark:bg-emerald-500/15 dark:text-emerald-200">
                        ...
                        <span>Sem atrasos imediatos</span>
                    </span>
                @endif

                @if((int) $summary['vence_hoje'] > 0)
                    <span class="inline-flex items-center gap-1.5 rounded-full border border-amber-300/80 bg-amber-50 px-2.5 py-0.5 font-medium text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/15 dark:text-amber-200">
                        ...
                        <span><strong class="font-semibold">{{ $summary['vence_hoje_formatted'] }}</strong> vencem hoje</span>
                    </span>
                @endif
            </div>
        </x-slot>
        ...
        <div class="space-y-4">
            {{-- Faixa de Pontos de Atenção Imediata (quando houver itens críticos) --}}
            @if($hasAlerts)
                <div class="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3.5 dark:border-amber-500/20 dark:bg-[#0a2028]/60">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex items-center gap-2.5">
                            <span class="flex size-7 shrink-0 items-center justify-center rounded-lg bg-amber-500/15 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300">
                                <x-heroicon-m-exclamation-triangle class="size-4" aria-hidden="true" />
                            </span>
                            <div>
                                <h4 class="text-xs font-semibold uppercase tracking-wider text-amber-800 dark:text-amber-300">
                                    Pontos de Atenção Imediata
                                </h4>
                                <p class="text-[0.6875rem] text-gray-600 dark:text-gray-400">
                                    Itens que exigem resolução prioritária ou atribuição de responsável.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            @foreach($alerts as $alert)
                                @if((int) $alert['raw_value'] > 0)
                                    <div class="inline-flex items-center gap-2 rounded-lg border {{ $alert['border_class'] }} {{ $alert['bg_class'] }} px-2.5 py-1 text-xs">
                                        <span class="size-1.5 rounded-full {{ $alert['dot_class'] }}"></span>
                                        <span class="font-bold tabular-nums {{ $alert['text_class'] }}">{{ $alert['value'] }}</span>
                                        <span class="text-gray-700 dark:text-gray-300">{{ $alert['label'] }}</span>
                                    </div>
                                @endif
                            @endforeach
                        </div>
            ...
            {{-- 1. Nível Primário: KPIs Operacionais Críticos (Grid 4 colunas) --}}
            <div>
                <div class="mb-2 flex items-center justify-between">
                    ...
                    <span class="text-[0.6875rem] text-gray-400 dark:text-gray-500">
                        Ação Imediata & Janelas Críticas
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach($cards as $card)
                        @php
                            ...
                            $toneClasses = match ($tone) {
                                ...
                                'primary' => [
                                    ...
                                    'badge' => 'border border-gray-200 bg-bsi-paper text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-300',
                                    'value' => 'text-gray-950 dark:text-white',
                                    'desc' => 'text-gray-600 dark:text-gray-300',
                                ],
                                default => [
                        ...
                        >
                            ...
                            {{-- Meio: Número Principal em Alto Contraste (Off-White) --}}
                            <div class="my-2.5">
                            ...
                            {{-- Rodapé: Descrição Auxiliar Neutra --}}
            ...
            {{-- 2. Nível Secundário & Contextual: Grade Compacta de Acompanhamento (8 cards) --}}
            <div>
                <div class="mb-2 flex items-center justify-between">
                    <span class="text-[0.6875rem] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        Indicadores de Acompanhamento & Contexto
                    </span>
                    <span class="text-[0.6875rem] text-gray-400 dark:text-gray-500">
                        Monitoramento Contínuo e Ciclos
                    </span>
                </div>
                    ...
                    @foreach($secondary as $item)
                        @php
                            ...
                            $itemClasses = match ($tone) {
                                ...
                                'info' => [
                                    'dot' => 'bg-info-500',
                                    'border' => 'border-info-300/40 dark:border-info-500/20',
                                    'tag' => 'text-info-700 dark:text-info-300',
                                ],
                                default => [
                                    'dot' => 'bg-gray-400',
