...
@php
    ...
    $groupBtnAttributes = new \Illuminate\View\ComponentAttributeBag([
        'class' => 'fi-sidebar-group-btn',
    ]);

    if ($collapsible) {
        $groupBtnAttributes = $groupBtnAttributes->merge([
    ...
    $groupItemsAttributes = new \Illuminate\View\ComponentAttributeBag([
        'class' => 'fi-sidebar-group-items',
    ]);

    if (filled($label)) {
        $groupItemsAttributes = $groupItemsAttributes->merge([
            'id' => $groupItemsId,
            'x-show' => $sidebarCollapsible ? '$store.sidebar.isOpen ? ! isCollapsed() : ! ' . ($hasDropdown ? 'true' : 'false') : '! isCollapsed()',
            'x-collapse.duration.200ms' => true,
        ]);
    }
...
<li
    x-data="{
        ...
        isCollapsed() {
            ...
            return $store.sidebar.groupIsCollapsed(this.label);
        }
    }"
    data-group-label="{{ $subNavigation ? "sub_navigation_{$label}" : $label }}"
    x-init="if (isActive && $store.sidebar.groupIsCollapsed(label)) { $store.sidebar.toggleCollapsedGroup(label); }"
    x-bind:class="{ 'fi-collapsed': isCollapsed() }"
    {{ $liAttributes }}
>
    @if ($label)
        <div {{ $groupBtnAttributes }}>
            @if ($icon)
