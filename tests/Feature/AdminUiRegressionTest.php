...
it('renders a branded microsoft-only admin access flow', function () {
    $this->get('/admin/login')
        ...
        ->assertSee('Acesso corporativo seguro e controlado.')
        ->assertSee('Este ambiente é destinado exclusivamente a usuários autorizados')
        ->assertSee('Governança & Conformidade')
        ->assertSee('Autenticação integrada ao Microsoft 365 com Single Sign-On (SSO)')
        ->assertSee('Acesso corporativo')
        ->assertSee('Autenticação segura')
        ->assertSee('Ambiente administrativo')
        ->assertSee('Entrar no sistema')
        ->assertSee('Utilize sua conta corporativa autorizada')
        ->assertSee('Entrar com Microsoft 365')
        ->assertSee('Single Sign-On Corporativo (SSO)')
        ->assertSee('Acesso restrito a usuários autorizados. Uso interno.')
        ->assertSee('Fale com o suporte')
        ->assertSee('mailto:contato@bsicapital.com.br')
        ->assertSee('Portal Corporativo')
